<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
<html class="uk-height-1-1 uk-notouch">
<head>
<meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<link rel="shortcut icon"
	href="https://getuikit.com/v2/docs/images/favicon.ico"
	type="image/x-icon">
<link rel="apple-touch-icon-precomposed"
	href="https://getuikit.com/v2/docs/images/apple-touch-icon.png">
<link rel="stylesheet" href="css/uikit.docs.min.css">
<script
	src="https://ajax.googleapis.com/ajax/libs/jquery/3.2.1/jquery.min.js"></script>
<script
	src="https://cdnjs.cloudflare.com/ajax/libs/uikit/3.0.0-beta.20/js/uikit.min.js"></script>
<style type="text/css">
body {
	background-image: url(css/images/background.jpg);
	height: 100%;
	width: 100%;
	background-size: cover;
}
</style>
<title>登入</title>
</head>
<body>
<body class="uk-height-1-1">
	<div class="uk-vertical-align uk-text-center uk-height-1-1">
		<div class="uk-vertical-align-middle" style="width: 250px;">
			<img class="uk-margin-bottom" width="140" height="120"
				src="data:image/svg+xml;base64,"
				alt="">
			
			<form class="uk-panel uk-panel-box uk-form" method="post"
				action="login.do">
				<div class="uk-form-row">
					<input class="uk-width-1-1 uk-form-large" type="text" name="userId"
						id="usr" placeholder="Username">
				</div>
				<div class="uk-form-row">
					<input class="uk-width-1-1 uk-form-large" type="password"
						name="pswd" id="pwd" placeholder="Password">
				</div>
				<div class="uk-form-row">
					<a type="submit"
						class="uk-width-1-1 uk-button uk-button-primary uk-button-large"
						id="btnLogin">Login</a>
				</div>
				<div class="uk-form-row uk-text-small">
					<label class="uk-float-left"><input type="checkbox"
						name="rememberMe"> Remember Me</label> 
						<a class="uk-float-right uk-link uk-link-muted"
						href="https://getuikit.com/v2/docs/layouts_login.html#">Forgot
						Password?</a>
				</div>
			</form>
		</div>
	</div>
	<script type="text/javascript">
		$('#btnLogin').click(function(event) {
			$.post('login.do', {
				'userId' : $('#usr').val(),
				'pswd' : $('#pwd').val(),
				'rememberMe' : null
			}).done(function() {
				alert('success');
			}).fail(function() {
				alert('error');
			})
		});
	</script>

</body>
</html>